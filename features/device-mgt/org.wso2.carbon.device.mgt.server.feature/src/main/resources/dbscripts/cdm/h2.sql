...
CREATE TABLE IF NOT EXISTS DM_DEVICE_TYPE_SENSOR (
    SENSOR_ID           INTEGER AUTO_INCREMENT  NOT NULL,
    DEVICE_TYPE_ID      INTEGER                 NOT NULL,
    SENSOR_NAME         VARCHAR(300)            NOT NULL,
    DESCRIPTION         TEXT                    DEFAULT NULL,
    SENSOR_TYPE         VARCHAR(300)            DEFAULT NULL,
    STREAM_DEFINITION   TEXT                    DEFAULT NULL,
    PRIMARY KEY (SENSOR_ID),
    CONSTRAINT fk_DM_DEVICE_TYPE_SENSOR_DM_DEVICE_TYPE
        FOREIGN KEY (DEVICE_TYPE_ID)
        REFERENCES DM_DEVICE_TYPE (ID)
        ON DELETE NO ACTION
        ON UPDATE NO ACTION,
    CONSTRAINT uc_DEVICE_TYPE_TO_SENSOR_NAME UNIQUE (DEVICE_TYPE_ID,SENSOR_NAME)
);

CREATE TABLE IF NOT EXISTS DM_DEVICE_TYPE_SENSOR_STATIC_PROPERTIES (
    SENSOR_PROPERTY_ID   INTEGER AUTO_INCREMENT NOT NULL,
    SENSOR_ID            INTEGER                NOT NULL,
    PROPERTY_KEY         VARCHAR(500)           NOT NULL,
    PROPERTY_VALUE       VARCHAR(500)           DEFAULT NULL,
    PRIMARY KEY (SENSOR_PROPERTY_ID),
    CONSTRAINT fk_DM_DEVICE_TYPE_SENSOR_ID_PROPERTY
        FOREIGN KEY (SENSOR_ID)
        REFERENCES DM_DEVICE_TYPE_SENSOR (SENSOR_ID)
        ON DELETE NO ACTION
        ON UPDATE NO ACTION,
    CONSTRAINT uc_DEVICE_TYPE_SENSOR_ID_TO_PROP_KEY UNIQUE (SENSOR_ID,PROPERTY_KEY)
);

...
public class DeviceManagementProviderServiceImpl implements DeviceManagementProviderService,
	...
	public boolean saveConfiguration(TenantConfiguration configuration) throws DeviceManagementException {
		DeviceManager dms =
				this.getPluginRepository().getDeviceManagementService(configuration.getType(), this.getTenantId())
						.getDeviceManager();
	...
	private List<EnrolmentInfo> getEnrollmentsOfUser(int deviceId, String user)
		...
		try {
			...
			throw new DeviceManagementException("Error occurred while obtaining the enrollment information device " +
                                                        "for" +
	...
	public void sendRegistrationEmail(EmailMessageProperties emailMessageProperties) throws DeviceManagementException {
		...
		try {
			...
			messageBody = messageBody.trim().replaceAll("\\{" + EmailConstants.EnrolmentEmailConstants
																.USERNAME
																+ "\\}",
														URLEncoder.encode(emailMessageProperties.getUserName(),
																		  EmailConstants.EnrolmentEmailConstants
																				  .ENCODED_SCHEME));
			messageBody = messageBody.trim().replaceAll("\\{" + EmailConstants.EnrolmentEmailConstants.DOMAIN
																+ "\\}",
														URLEncoder.encode(emailMessageProperties.getDomainName(),
																		  EmailConstants.EnrolmentEmailConstants
																				  .ENCODED_SCHEME));
			messageBody = messageBody.replaceAll("\\{" + EmailConstants.EnrolmentEmailConstants.PASSWORD + "\\}",
												 URLEncoder.encode(emailMessageProperties.getPassword(),
																   EmailConstants.EnrolmentEmailConstants
																		   .ENCODED_SCHEME));
	...
	public List<DeviceType> getAvailableDeviceTypes() throws DeviceManagementException {
		List<DeviceType> deviceTypesInDBForSpecificTenant;
		List<DeviceType> publicSharedDeviceTypesInDB;
		List<DeviceType> deviceTypesResponse = new ArrayList<>();
		try {
			DeviceManagementDAOFactory.openConnection();
			int tenantId = this.getTenantId();
			deviceTypesInDBForSpecificTenant = deviceTypeDAO.getPrivateDeviceTypes(tenantId);
			...
			if (registeredTypes != null) {
				if (deviceTypesInDBForSpecificTenant != null) {
					for (DeviceType deviceType : deviceTypesInDBForSpecificTenant) {
	...
	@Override
	public String getDeviceTypeTenantProvider(String type) throws DeviceManagementException {
		DeviceManagementService deviceManagementService = pluginRepository.getDeviceManagementService(type,
																									  this.getTenantId());
		String tenantDomain = null;
		if (deviceManagementService != null) {
			tenantDomain = deviceManagementService.getProviderTenantDomain();
			return tenantDomain;
		} else {
			throw new DeviceManagementException("Tenant does not have a device type" + type);
		}
	}
	...
	@Override
	public void updateDeviceEnrolmentInfo(Device device, EnrolmentInfo.Status status) throws

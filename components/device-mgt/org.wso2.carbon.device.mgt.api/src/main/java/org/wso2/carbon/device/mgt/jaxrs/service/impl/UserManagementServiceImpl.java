    ...
    public Response getUsers(@QueryParam("filter") String filter,
        ...
        try {
            ...
            for (String username : users) {
                ...
                user.setUserDeviceCount(dms.getDeviceCount(username));
                userList.add(user);
            }
            if(displayName == null){
            	displayName = "";
            ...
            if(!displayName.isEmpty()) {
            	...
            	ListIterator<BasicUserInfo> itr = userList.listIterator();
            	while(itr.hasNext()) {
            		BasicUserInfo tempUser = itr.next();
            		if(tempUser.getUsername().equalsIgnoreCase("admin"))
            			tempDisplayName = tempUser.getUsername();
            		else
            			tempDisplayName = tempUser.getFirstname() + " " + tempUser.getLastname();
            		if(!tempDisplayName.toLowerCase().startsWith(displayName.toLowerCase()))
            			itr.remove();
            	}
            }
        	...
        	String msg = "Error occurred while retrieving device count for the list of users.";
            log.error(msg, e);
            return Response.serverError().entity(
                    new ErrorResponse.ErrorResponseBuilder().setMessage(msg).build()).build();
		}
...
@Override
	 @POST
	 @Path("/deleteUsers")
	public Response removeUsers(List<String> userIds, @QueryParam("domain") String domain) {
		 ...
		 String userNm=null;
	        try {
	            ...
	            String msg = "Exception in trying to remove user by username: " + userNm;
	            log.error(msg, e);
	            return Response.serverError().entity(
	                    new ErrorResponse.ErrorResponseBuilder().setMessage(msg).build()).build();
	        }

...
CREATE TABLE DM_SHARED_DEVICE_TYPE (
  DEVICE_TYPE_ID NUMBER(10) NOT NULL,
  SHARED_TENANT_ID NUMBER(10) DEFAULT 0,
  ...
  CONSTRAINT PK_DM_SHARED_DEVICE_TYPE PRIMARY KEY (DEVICE_TYPE_ID)
)
/
-- Generate ID using sequence and trigger
CREATE SEQUENCE DM_SHARED_DEVICE_TYPE_seq START WITH 1 INCREMENT BY 1 NOCACHE
/
CREATE OR REPLACE TRIGGER DM_SHARED_DEVICE_TYPE_seq_tr
BEFORE INSERT
ON DM_SHARED_DEVICE_TYPE
REFERENCING NEW AS NEW
FOR EACH ROW
WHEN (NEW.DEVICE_TYPE_ID IS NULL)
  BEGIN
    SELECT DM_SHARED_DEVICE_TYPE_seq.NEXTVAL INTO :NEW.DEVICE_TYPE_ID FROM DUAL;
  END;
/

...
public class RoleManagementServiceImpl implements RoleManagementService {
        ...
        for (String role : roles) {
            if (!(role.startsWith("Internal/") || role.startsWith("Authentication/") || role.startsWith(
                ...
                if (!filterRolesByName) {
                    ...
                    if (role.toLowerCase().contains(filter.toLowerCase())) {
                        filteredRoles.add(role);
                    }
                }
            }
	...
	@Override
	 @POST
     @Path("/deleteRoles")
	public Response deleteRoles(List<String> roleNames, @QueryParam("user-store") String userStoreName) {
		String rolNm=null;
		try {
        ...
        String msg = "Error occurred while deleting the role '" + rolNm + "'";
        log.error(msg, e);
        return Response.serverError().entity(
                new ErrorResponse.ErrorResponseBuilder().setMessage(msg).build()).build();
    }
	...
	@Override
	public Response getUsersofRole(@PathParam("roleName") String roleName) {
		try {
			//int tenantId = CarbonContext.getThreadLocalCarbonContext().getTenantId();
			UserStoreManager userStore =DeviceMgtAPIUtils.getUserStoreManager();
			String[] users=userStore.getUserListOfRole(roleName);
			return Response.status(Response.Status.OK).entity(users).build();
		} catch (UserStoreException e) {
			e.printStackTrace();
			String msg = "Error Occured while retreving Users";

function triggerOperation(selection) {
    var content = $("#error_message").find(".content");
    var error_title = content.find("#title");
    var error_statusIcon = content.find("#status-icon");
    var error_description = content.find("#description");
    var deviceId = getSelectedDeviceIds();
    var deviceStatuses = getSelectedDeviceStatuses();
    for (var i = 0; i < deviceStatuses.length; i++) {
        if (deviceStatuses[i] != 'ACTIVE') {
            error_title.html("Action Cannot be performed!");
            error_description.html("<p> You cannot perform the action on devices which are not active.Please select only active devices to perform the action </p>");
            $(modalPopupContent).addClass("operation-data");
            $(modalPopupContent).html($("#error_message").html());
            showPopup();
            return;
        }
    }
    if (deviceId.length == 0) {
        error_title.html("Action Cannot be performed!");
        error_description.html("<p> No Devices Selected! Please select a device or multiple devices to perform the action</p>");
        $(modalPopupContent).addClass("operation-data");
        $(modalPopupContent).html($("#error_message").html());
        showPopup();
    } else {

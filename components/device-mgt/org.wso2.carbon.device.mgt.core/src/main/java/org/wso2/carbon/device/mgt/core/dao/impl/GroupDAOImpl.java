...
public class GroupDAOImpl implements GroupDAO {
    ...
    public List<DeviceGroup> getGroups(GroupPaginationRequest request, int tenantId)
        ...
        try {
            ...
            String sql ="SELECT dm.ID, dm.DESCRIPTION, dm.GROUP_NAME, dm.OWNER, COUNT(dm_map.ID) AS DEVICE_COUNT " + 
            			"FROM  DM_GROUP dm LEFT JOIN  (SELECT dgm.ID, dgm.DEVICE_ID, dgm.GROUP_ID, dgm.TENANT_ID " + 
            			"FROM DM_DEVICE_GROUP_MAP dgm LEFT JOIN DM_ENROLMENT a ON a.DEVICE_ID=dgm.DEVICE_ID " + 
            			"WHERE a.STATUS != 'REMOVED') dm_map ON dm_map.GROUP_ID=dm.ID " + 
            			"WHERE  dm.TENANT_ID = ? ";
            ...
            while (resultSet.next()) {
                deviceGroupList.add(GroupManagementDAOUtil.loadGroupWithDeviceCount(resultSet));
            }
        } catch (SQLException e) {
    ...
    //Method Overloaded
    public int getDeviceCount(int groupId, int tenantId, String name, String user, String status, String deviceType, String ownership, String model) throws GroupManagementDAOException {
        PreparedStatement stmt = null;
        ResultSet resultSet = null;
        try {
            Connection conn = GroupManagementDAOFactory.getConnection();
            //StringBuilder sql = new StringBuilder("SELECT COUNT(ID) AS DEVICE_COUNT FROM DM_DEVICE_GROUP_MAP WHERE GROUP_ID = ? AND TENANT_ID = ?");
            
            StringBuilder sql = new StringBuilder("SELECT COUNT(d1.DEVICE_ID) AS DEVICE_COUNT FROM DM_ENROLMENT e, " + 
            		"(SELECT gd.DEVICE_ID, gd.DESCRIPTION, gd.NAME, gd.DEVICE_IDENTIFICATION, t.NAME AS DEVICE_TYPE " + 
            		"FROM " + 
            		"(" + 
            		"SELECT d.ID AS DEVICE_ID, d.DESCRIPTION, d.NAME, d.DEVICE_IDENTIFICATION, d.DEVICE_TYPE_ID,a.DEVICE_MODEL FROM DM_DEVICE d, " + 
            		"WSO2MobileAndroid_DB.AD_DEVICE a,  (" + 
            		"SELECT dgm.DEVICE_ID FROM DM_DEVICE_GROUP_MAP dgm WHERE dgm.GROUP_ID = ?) dgm1 " + 
            		"WHERE d.ID = dgm1.DEVICE_ID and d.Device_IDENTIFICATION=a.DEVICE_ID AND d.TENANT_ID = ?");
            		
	            if(!model.equalsIgnoreCase("all"))
	            	sql.append(" AND LOWER(a.DEVICE_MODEL) = LOWER(?)");
 
            		sql.append( ") gd, DM_DEVICE_TYPE t WHERE gd.DEVICE_TYPE_ID = t.ID) d1 WHERE d1.DEVICE_ID = e.DEVICE_ID AND e.STATUS != 'REMOVED' AND TENANT_ID = ?");
            
            int tempIndex = 2;
           
            if(!name.isEmpty())
            	sql.append(" AND LOWER(d1.NAME) LIKE LOWER(?)");
            if(!user.isEmpty())
            	sql.append(" AND LOWER(e.OWNER) LIKE LOWER(?)");
            if(!status.isEmpty())
            	sql.append(" AND LOWER(e.STATUS) LIKE LOWER(?)");
            if(!deviceType.isEmpty())
            	sql.append(" AND LOWER(d1.DEVICE_TYPE) LIKE LOWER(?)");
            if(!ownership.isEmpty())
            	sql.append(" AND LOWER(e.OWNERSHIP) LIKE LOWER(?)");
            
            stmt = conn.prepareStatement(sql.toString());
            stmt.setInt(1, groupId);
            stmt.setInt(2, tenantId);
            if(!model.equalsIgnoreCase("all"))
            	stmt.setString(++tempIndex, model);
            
            stmt.setInt(++tempIndex, tenantId);
            if(!name.isEmpty())
            	stmt.setString(++tempIndex, name+"%");
            if(!user.isEmpty())
            	stmt.setString(++tempIndex, user+"%");
            if(!status.isEmpty())
            	stmt.setString(++tempIndex, status+"%");
            if(!deviceType.isEmpty())
            	stmt.setString(++tempIndex, deviceType+"%");
            if(!ownership.isEmpty())
            	stmt.setString(++tempIndex, ownership+"%");
    ...
    @Override
    public List<Device> getDevices(int groupId, int startIndex, int rowCount, int tenantId, String name, String user, String status, String deviceType, String ownership, String model)
            throws GroupManagementDAOException {
        Connection conn;
        PreparedStatement stmt = null;
        ResultSet rs = null;
        ...
        try {
            conn = GroupManagementDAOFactory.getConnection();
            StringBuffer sql = new StringBuffer("SELECT d1.DEVICE_ID, d1.DESCRIPTION, d1.NAME AS DEVICE_NAME, d1.DEVICE_TYPE, " +
                    ...
                    " DM_DEVICE d,WSO2MobileAndroid_DB.AD_DEVICE a, (" +
                    "SELECT dgm.DEVICE_ID FROM DM_DEVICE_GROUP_MAP dgm WHERE dgm.GROUP_ID = ?) dgm1 " +
                    "WHERE d.ID = dgm1.DEVICE_ID AND d.Device_IDENTIFICATION=a.DEVICE_ID AND d.TENANT_ID = ?");
                    
                    if(!model.equalsIgnoreCase("all"))
                    	sql.append(" AND a.DEVICE_MODEL = ? ");
            ...
            stmt.setInt(++tempIndex, startIndex);
            //noinspection JpaQueryApiInspection
            stmt.setInt(++tempIndex, rowCount);
            rs = stmt.executeQuery();
            devices = new ArrayList<>();
            while (rs.next()) {
                Device device = DeviceManagementDAOUtil.loadDevice(rs);
                DeviceManager deviceManager = this.getDeviceManager(device.getType(), tenantId);
                Device tempDevice;
				try {
					tempDevice = deviceManager.getDevice(new DeviceIdentifier(device.getDeviceIdentifier(), device.getType()));
					if(tempDevice != null) {
						device.setProperties(tempDevice.getProperties());
	                	device.setModel(tempDevice.getProperties().get(5).getValue());
	                }
				} catch (DeviceManagementException e) {
					e.printStackTrace();
				}
                if(model.equalsIgnoreCase("all")||device.getModel().equalsIgnoreCase(model))
                	devices.add(device);
    ...
    private DeviceManager getDeviceManager(String deviceType,int tenantId) {
    	DeviceManagementPluginRepository pluginRepository = getDeviceManagementService().getPluginRepository();
    	DeviceManagementService deviceManagementService=pluginRepository.getDeviceManagementService(deviceType, tenantId);
    ...
    @Override
    public List<Device> getDevices(int groupId, int startIndex, int rowCount, int tenantId) throws GroupManagementDAOException {
    	Connection conn;
        PreparedStatement stmt = null;
        ResultSet rs = null;
        List<Device> devices = null;

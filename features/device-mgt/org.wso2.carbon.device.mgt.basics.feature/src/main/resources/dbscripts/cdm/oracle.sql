...
-- Generate ID using sequence and trigger
CREATE SEQUENCE DM_DEVICE_HIERARCHY_seq START WITH 1 INCREMENT BY 1;

CREATE OR REPLACE TRIGGER DM_DEVICE_HIERARCHY_seq_tr
 BEFORE INSERT ON DM_DEVICE_HIERARCHY FOR EACH ROW
 WHEN (NEW.ID IS NULL)
BEGIN
 SELECT DM_DEVICE_HIERARCHY_seq.NEXTVAL INTO :NEW.ID FROM DUAL;
END;
/
